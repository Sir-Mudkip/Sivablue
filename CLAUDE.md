...
- **Source build** — one stage per project (`11-ghostty.sh`). Resolve the newest version upstream, verify the download's signature or checksum, build, then remove every build dep the stage added. **Pin the CPU baseline explicitly** (`-Dcpu=x86_64_v2` for Zig, `-march=x86-64-v2` for C): left to itself a build system optimises for the runner and the image `SIGILL`s on user hardware. Scratch space goes in `/var/tmp` — `/tmp` is a tmpfs (RAM) in the build `RUN`. Add a check to `99-tests.sh`, since `rpm -q` can vouch for none of it.
