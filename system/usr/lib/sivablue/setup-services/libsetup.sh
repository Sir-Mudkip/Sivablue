...
function version-script() {
  ...
  # concurrent setup scripts cannot read the JSON before either has written
  # back, which would run the same hook twice.
  (
    flock -x 200
    ...
    # Guard the write: an unchecked `mv` would move an empty or partial file
    # over the versioning database when jq fails, wiping every recorded
    # version and re-running every hook.
